...
// ─── Multi-Agent Crew UI ──────────────────────────────────────────────────────
// The Orchestrator Agent orchestrates 3 specialist agents via MCP tool calls.
// Each specialist agent is itself an MCP tool served from /api/mcp/agents.
...
export default function MultiAgentCrew() {
      ...
      <div className="bg-white dark:bg-zinc-800 border-b border-gray-200 dark:border-zinc-700 px-6 py-4">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-full bg-gradient-to-br from-blue-500 via-purple-500 to-emerald-500 flex items-center justify-center text-white text-sm">
              🤖
            </div>
            <div>
              <h2 className="text-sm font-semibold text-gray-900 dark:text-white">
                Orchestrator Agent + Marketing Crew
              </h2>
            ...
            <div className="text-center py-12">
              ...
              <div className="flex items-center justify-center gap-1 mb-8">
                ...
                <div className="flex flex-col items-center gap-2">
                  ...
                  <span className="text-xs font-medium text-indigo-700 dark:text-indigo-300">Orchestrator Agent</span>
                </div>
                {/* Arrow out */}
              ...
              <p className="text-sm text-gray-500 dark:text-zinc-500 max-w-lg mx-auto mb-3">
                An <span className="font-medium text-indigo-600 dark:text-indigo-400">Orchestrator Agent</span> orchestrates 3 specialist agents via MCP tool calls. It decides which agents to invoke, in what order, and passes outputs between them.
              </p>
              <div className="text-xs text-gray-400 dark:text-zinc-600 font-mono bg-gray-100 dark:bg-zinc-800 rounded-lg p-3 max-w-sm mx-auto text-left mb-6">
                <div>Orchestrator → researcher_agent(topic)</div>
                <div>Orchestrator → writer_agent(topic, research)</div>
                <div>Orchestrator → editor_agent(draft)</div>
                <div>Orchestrator → final response</div>
              </div>
      ...
      <div className="bg-white dark:bg-zinc-800 border-t border-gray-200 dark:border-zinc-700 px-4 py-4">
        <div className="max-w-4xl mx-auto">
          ...
          <p className="text-xs text-gray-400 dark:text-zinc-600 mt-2 text-center">
            Orchestrator Agent at /api/multi-agent · Specialist agents served via MCP at /api/mcp/agents · Database tools at /api/mcp/database
          </p>
        </div>
      </div>
    </div>
